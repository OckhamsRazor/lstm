...
function load_data_into_embs(fname)
  local embs = {}      --  n * 4*b*1000
  local emb = {}       --  4 * b*1000
  local emb_layer = {} --  b * 1000

  local file = torch.DiskFile(fname, 'r'):binary()
  file:quiet()
  repeat
...
local function summarize(embs)
  local summ = {} -- 4*b*1000
  local summ_layer = {} -- b*1000
  local rnn = {} -- 1000
  ...
  for l = 1, 2*params.layers do
    for b = 1, params.batch_size do
      for r = 1, params.rnn_size do
        rnn[r] = 0
        for i = 1, #embs do
          rnn[r] = rnn[r] + tonumber(embs[i][l][b][r]) --/ #embs
        end
      end
      table.insert(summ_layer, rnn)
      rnn = {}
    end
    table.insert(summ, summ_layer)
    summ_layer = {}
  end
...
local function main()
  ...
  for filename in paths.iterfiles(opt.input) do
    ...
    state_input = {data=summarize(load_data_into_embs(
